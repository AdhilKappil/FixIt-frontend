...
console.log(props.bookingId,"booking");
    ...
    const verifyEmailOtpHandler = async()=>{
       try {
        const res = await verifyEmailOtp({otp,email:props.conversationData.userEmail,
    ...
    return (
     <div className='flex justify-center mt-10'>
      ...
      <div className='flex justify-center mt-5'> {timer > 0 && `Resend OTP in ${timer} seconds`}</div>
      <div className='flex justify-center my-5'>
        {showResendButton ? 
         <button onClick={resendOtp} className='bg-primary p-2 w-28 text-white rounded'>Resend OTP</button>
        : <button onClick={verifyEmailOtpHandler} className='bg-primary p-2 w-28 text-white rounded'>Verify</button>
        }
      </div>
